...
![rossman](/rossmann-pictures/box_plot.png)
...
We can use Time Series do predict the sales for each store in the next 42 days (six weeks). The delivered solution was a Telegram bot.
...
Data Preparation In this part of the project, we did the data preparation to model our data for the ML training, as the most ML algorithms takes advantages of numerical data.

Key points:

There are 3 types of data preparation:
Normalization: rescales the center to 0 with standard deviation equal to 1. It is usually applied to data that is Normally distributed.

Rescaling: rescales the data to the interval from 0 to 1. It is usually applied to data that are non-Normally distributed. We can use two techniques for this type.

Min-Max Scaler: uses the data range.
Robust Scaler: uses the data IQR.
Transformation: transforms the data through encoding, magnitude or nature.
...
.1 Normalization We need to check the variables distributions.
...
Here, I used the sklearn.preprocessing.RobustScaler because of the presence of strong outliers.
...
Here, I used the sklearn.preprocessing.MinMaxScaler despite the presence of outliers, they are not that far from the superior whisker. So we can take a chance to use the Min-Max Scaler.
...
"Entities should not be multiplied without necessity." "Plurality is not to be posited without necessity"

It is variously paraphrased by statements like "the simplest solution is most likely the right one".

If you want to know more about it check: https://en.wikipedia.org/wiki/Occam%27s_razor
...
Filter Methods: takes advantage of the correlation between variables and its types (numerical and categorical).


Embedded Methods: takes advantage of algorithms, such as: Random Forest, Lasso Regression and Ridge Regression.

Wrapper Methods: it's a process and we can make use of Boruta's algorithm.
...
Comparing models' performance In this first comparison, before Cross Validation, we divided our dataset in two: training and test datasets.
...
For this project, we used Heroku to deploy our model.


10.1 Creating a bot in Telegram
First, to create our bot, we need to talk to a... bot! Oh yeah! the BotFather who is responsible for assisting us in the creation of a bot for Telegram. I won't describe the process in details here because the documentation from Telegram is pretty complete and easy to follow.
